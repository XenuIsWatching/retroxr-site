...
##
## Omit --subject to render every one. Frames land in res://probe_out/props/<name>/.
## Throwaway — delete the .gd/.tscn/.uid once the clips are encoded.
extends Node

const OUT_ROOT := "res://probe_out/props"
...
const SUBJECTS := {
	"motion_plus": {"path": "res://Scenes/Objects/controllers/wii/motion_plus.tscn"},
	"ray_gun": {"path": "res://Scenes/Objects/peripherals/ray_gun.tscn"},
}
...
func _shoot(name: String, spec: Dictionary) -> void:
	...
	if spec.has("mesh"):
		...
		if m == null:
			print("[props] %s: cannot load mesh %s" % [name, spec["mesh"]])
			return
		...
		if packed == null:
			print("[props] %s: cannot load %s" % [name, spec["path"]])
			return
	...
	if box.size == Vector3.ZERO and not focus.is_empty():
		print("[props] %s: focus matched nothing, falling back to whole scene" % name)
		box = _accumulate(node, AABB(), [false], [])
	if box.size == Vector3.ZERO:
		print("[props] %s: no visuals" % name)
	...
	print("[props] %-22s %d/%d  extent=%.3f m  radius=%.3f m"
